...
from app.agents.negotiation_agent import run_negotiation_agent, should_accept_vendor_price
...
@pytest.mark.asyncio
async def test_full_project_smoke_flow(monkeypatch):
    event_id = uuid.uuid4()
...
@pytest.mark.asyncio
async def test_event_creation_route_schedules_background_pipeline(monkeypatch):
    fake_db = FakeEventDB()
    ...
    async def fake_run_full_pipeline(*args, **kwargs):
        return None
    ...
    user = types.SimpleNamespace(uid="customer-123", email="customer@example.com")
    request = Request(
        {
            "type": "http",
            "method": "POST",
...
def test_core_budget_and_pricing_features():
    asking_price, floor_price = calculate_vendor_event_price("Caterer", 2000, 1500, 220)
    assert asking_price == 440000
    assert floor_price == 330000

    assert should_accept_vendor_price(240000, 250000, 275000) is True
    assert should_accept_vendor_price(280000, 250000, 275000) is False
